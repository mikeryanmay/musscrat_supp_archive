cat("writing Rev scripts for simulation 4.\n")
...
reps       = 100
runs       = 2
# state_dep  = c("state_dependent", "state_independent")
state_dep  = "state_dependent"
noise      = "constant_rate"

dir.create("analyses/simulation_study/simulation_4/jobs", showWarnings=FALSE)
...
# read the stub
lines = readLines("analyses/simulation_study/simulation_4/src/template.Rev")

# setup the SLURM script info
batch_scripts = list()
script_names  = c()
num_jobs      = 0
num_scripts   = 0
...
for(i in 1:nrow(grid)) {
  ...
  this_file = paste0("analyses/simulation_study/simulation_4/jobs/job_",i,".Rev")
