...
regions = c("16s","COI","CYTB","RAG1","RAG2","S7","TMO")
...
for(i in 1:length(regions)) {
  ...
  this_file = paste0("analyses/empirical_analyses/haemulids/step_1/output/",this_region,"_unrooted/pps_statistics.csv")
  ...
  if(file.exists(this_file)) {
    ...
    pdf(paste0("analyses/empirical_analyses/haemulids/step_1/output/",this_region,"_unrooted/pps_summary.pdf"), height=5, width=5 * num_subsets)
    layout(layout_mat)
    for(j in 1:num_subsets) {

      # get the relevant statistics
      this_obs_stat = as.numeric(these_obs_stats[j])
      if(class(these_sim_stats) == "numeric") {
        this_sim_stat = these_sim_stats
      } else {
